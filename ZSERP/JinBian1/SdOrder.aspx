    ...
    <script language="javascript" type="text/javascript">
        ...
        $(function () {
            if (Page.usetype == "modify" || Page.usetype == "view") {
                ...
                var sqlObj1 = {
                    TableName: "SDOrderD",
                    Fields: "sum(fOutTotal) as fOutTotal",
                    SelectAll: "True",
                    Filters: [
                        {
                            Field: "iMainRecNo",
            ...
            options.columns[0].splice(12, 0,
                {
                    title: "深加工用料",
                    field: "dddd",
                    formatter: function (value, row, index) {
                        if (row.iRecNo)
                            return "<a href='javascript:void(0)' onclick='openwin1(" + index + ")'>深加工用料</a>";
                    },
                    width: 70,
                    align: "center"
                }
            ...
            if (Page.usetype == "modify") {
                ...
                if (isModify == "1") {
                    ...
                    if (sModifyType == "客户信息变更") {
                        ...
                        Page.setFieldEnabled("iDay");
                        Page.setFieldEnabled("sProductAsk");
                        Page.setFieldEnabled("sReceiveAddr");
                        Page.setFieldEnabled("sTransType");
                        Page.setFieldEnabled("sPackType");
                        Page.setFieldEnabled("sCheckStand");
                        Page.setFieldEnabled("fMoney");
                        //                        Page.setFieldEnabled("iSubscription");
                        //                        Page.setFieldEnabled("sMiddleCostName");
                    }
        ...
        Page.beforeSave = function () {
            var type = Page.getFieldValue("iOrderType");  //判断类型为深加工
            if (type == 6)
        ...
        dataForm.onBeforeOpen = function (uniqueid) {
            ...
            if (uniqueid == "1278") {
                if (isModify == true && sModifyType != "颜色变更") {
                    alert("非颜色变更，不可从颜色档案转入！");
                    return false;
                }
            }
        }
        ...
        function openwin1(index) {
            var selectRow = $("#SDOrderD").datagrid("getRows")[index];
        ...
        Page.Children.onLoadSuccess = function (tableid, data) {
            if (tableid == "SDOrderD") {
                var fTotal = Page.getFieldValue("fTotal");
                var fReceiveTotal = Page.getFieldValue("fReceiveTotal");
                var fSendTotal = Page.getFieldValue("fSendTotal");
                var fLeftTotal = (isNaN(Number(fTotal)) ? 0 : Number(fTotal)) - (isNaN(Number(fReceiveTotal)) ? 0 : Number(fReceiveTotal));
                ...
                <tr>
                    <td colspan="2">
                        ...
                        <cc1:ExtTextBox2 ID="ExtTextBox17" runat="server" Z_FieldID="iMiddleCost" Style="width: 36px"
                            Z_FieldType="整数" />
                        %&nbsp; 尾款
                        <cc1:ExtTextBox2 ID="ExtTextBox25" runat="server" Z_FieldID="iEndCost" Style="width: 32px"
                            Z_FieldType="整数" />
                        %
                    </td>
                </tr>
