    ...
    <script language="javascript" type="text/javascript">
        ...
        lookUp.afterSelected = function (uniqueid, data) {
            if (uniqueid == "1197") {
                var sqlObj =
                {
                    TableName: "SDOrderDDVatNoDReelNo as a",
                    Fields: "fRealQty,fWeight,sReelNo,sBarcode",
                    SelectAll: "True",
                    Filters: [
                        {
                            Field: "iSDOrderMRecNo",
                            ComOprt: "=",   
                            Value: "'" + data.iSdContractMRecNo + "'",
                            LinkOprt: "and"
                        },
                        {
                            Field: "iSDOrderDRecNo",
                            ComOprt: "=",
                            Value: "'" + data.iSdContractDRecNo + "'",
                            LinkOprt: "and"
                        }
                        ,
                        {
                        ...
                        },
                        {
                            Field:"not ",
                            ComOprt: "exists ",
                            Value: "(select 1 from MMStockProductInM as b,MMStockProductInD as c where b.iRecNo=c.iMainRecNo and b.iStatus=4 and b.iRecNo<>'" + Page.key + "' and c.sBarcode=a.sBarcode)"
                        }
                    ],
                    Sorts: [
                        {
                ...
                $("#MMStockProductInD").datagrid("loadData", { rows: [], total: 0 });
                var iBscDataStockDRecNo = Page.getFieldValue("iBscDataStockDRecNo");
                var sBerChID = Page.getFieldText("iBscDataStockDRecNo");
                ...
                for (var i = 0; i < result.length; i++) {
                    var appRow = {};
                    appRow.sReelNo = result[i].sReelNo;
                    appRow.fPurQty = isNaN(Number(result[i].fWeight)) ? 0 : Number(result[i].fWeight) * isHc;
                    if (sUnitID == "M") {
                        appRow.fQty = isNaN(Number(result[i].fRealQty)) ? 0 : Number(result[i].fRealQty) * isHc;
                        appRow.sLetCode = (isNaN(Number(result[i].fRealQty)) ? 0 : Number(result[i].fRealQty) * isHc) * 0.9144;
                    }
                    else {
                        appRow.sLetCode = isNaN(Number(result[i].fRealQty)) ? 0 : Number(result[i].fRealQty) * isHc;
                        appRow.fQty = (isNaN(Number(result[i].fRealQty)) ? 0 : Number(result[i].fRealQty) * isHc) * 0.9144;
                    }

                    appRow.sBarCode = result[i].sBarcode;
                    appRow.sBerChID = sBerChID;
                    appRow.iBscDataStockDRecNo = iBscDataStockDRecNo;
                    Page.tableToolbarClick("add", "MMStockProductInD", appRow);
                }
            }
        }
    </script>
    ...
    <div id="divContent" class="easyui-layout" data-options="fit:true,border:false">
        <div data-options="region:'north',border:false" style="overflow: hidden;">
            ...
            <div id="divHiden" style="display: none;">
                ...
                <cc1:ExtTextBox2 ID="ExtTextBox8" Z_FieldID="iSdOrderDRecNo" runat="server" />
            </div>
            <!--主表部分-->
            <table class="tabmain">
                <tr>
                    ...
                    <td>
                        <cc1:ExtTextBox2 ID="ExtTextBox22" runat="server" Z_FieldID="sBatchNo" Width="150px" />
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <cc1:ExtTextBox2 ID="ExtTextBox21" runat="server" Z_FieldID="iSdOrderMRecNo" Width="150px"
                            Z_readOnly="true" />
                    </td>
                    <td>
                        会计月份
                    ...
                    <td>
                        <cc1:ExtTextBox2 ID="ExtTextBox7" runat="server" Z_FieldID="sUnitID" Style="width: 100px"
                            Z_NoSave="True" />
                    </td>
                </tr>
                <tr>
