...
$(function () {
    $.ajax({
        ...
        async: false,
        data: { otype: "getcurtuserid", random: Math.random() },
        success: function (data) {
            userid = data;
        }
    });
    LookUp = new LookUpObj();
    stype = getQueryString("itype");
    document.getElementById("ExtHidden1").value = stype;
    if (stype == "01") {
    ...
    if (stype != '01' && stype != '03') {
        ...
        var jsontree = {
            ...
            Fields: "iRecNo,sClassID,sClassName,sParentID,stype,sClassID+'-'+sClassName as sName",
            SelectAll: "True",
        ...
        if (stype == '01') {
            var sqltext = "select * from(" +
                           "select 0 as iRecNo,'01' as stype, '0' as sParentID,a.sCode as sClassID,isnull(a.sCode,'')+'-'+a.sName as sName,sName as sClassName from BscDataListD as a where a.sClassID='matClass' " +
                            "union " +
                            " select iRecNo,stype,sParentID,sClassID,sClassID+'-'+sClassName as sName,sClassName from bscDataClass where stype='01'" +
            ") as a order by sClassID";
            data = SqlGetDataComm(sqltext);
        }
        else if (stype == '03') {
            var sqltext = "select iRecNo,sClassID,sClassName,sParentID,stype,sClassID+'-'+sClassName as sName " +
                      "from bscDataClass where stype='03' " +
                      "union " +
                      "select 0 as iRecNo,sCode as sClassID,sName as sClassName,'0' as sParentID,'03' as stype," +
    ...
    $("#tree1").ligerTree({
        ...
        textFieldName: 'sName',
        attribute: ['iRecNo,stype,sClassName'],
...
function itemclick(item) {
    if (item.id == "addm") {
        ...
        if (selectedNode) {
            ...
            var sqltext = "select max(sClassID) as r from bscDataClass where stype='" + stype + "' and sParentID='" + selectedNode.data.sParentID + "'";
            var maxclassid = "";
            ...
            var sqltext = "select max(sClassID) as r from bscDataClass where stype='" + stype + "' and sParentID='0'";
            var maxclassid = "";
            var jsonresult = SqlGetDataComm(sqltext);
            if (jsonresult.length > 0) {
                var mclassid = jsonresult[0].r;
                    ...
                    if (confirm("确认删除所选择分类？")) {
                        ...
                        if (result == "1") {
                            ...
                            if (stype != '01' && stype != '03') {
                                ...
                                if (stype == '01') {
                                    var sqltext = "select * from(" +
                           "select 0 as iRecNo,'01' as stype, '0' as sParentID,a.sCode as sClassID,a.sCode+'-'+a.sName as sName,sName as sClassName from BscDataListD as a where a.sClassID='matClass' " +
                            "union " +
                            " select iRecNo,stype,sParentID,sClassID,sClassID+'-'+sClassName as sName,sClassName from bscDataClass where stype='01'" +
            ") as a order by sClassID";
                                    data = SqlGetDataComm(sqltext);
...
function f_treeSelect(node) {
    ...
    if (node.data.iRecNo == "0" && (stype == "01" || stype == "03")) {
