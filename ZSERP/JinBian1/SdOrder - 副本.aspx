    ...
    <script language="javascript" type="text/javascript">
        ...
        function openwin1(index) {
            ...
            if (result.length > 0) {
                var  type= Page.getFieldValue("iOrderType");  //判断类型为深加工
                if (type==6)
                    window.open("/ZSERP/JinBian/SDOrderDPart.aspx?iformid=8053&usetype=modify&key=" + iRecNo + "&r=" + Math.random(), 'newwindow', 'height=600,width=1150,top=(window.screen.availHeight-30-iHeight)/2,left=(window.screen.availWidth-10-iWidth)/2,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=no, status=no');
                else 
                {
                    alert("订单类型深加工才可点击");
                    return false;
