    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (otype == "GetFormInfo")
        {
            ...
            try
            {
                ...
                jsonStr.Append("\"sOpenFilters\":\"" + billData.Rows[0]["sOpenFilters"].ToString().Replace("{userid}", userid) + "\",");
                ...
                //",\"sDeitailFieldKey\":\"" + billData.Rows[0]["sDeitailFieldKey"].ToString() + "\",\"sOpenFilters\":\"" + billData.Rows[0]["sOpenFilters"].ToString().Replace("{userid}", userid) + "\"}";
        ...
        else if (otype == "GetFormData")
        {
            ...
            try
            {
                List<DataTable> dts = mf.GetFormListData(iformid, userid, rows, filters, isDetail);
                result.success = true;
                result.tables = dts;
            }
            catch (Exception ex)
        ...
        else if (otype == "GetFormDataStyle")
        {
            ...
            try
            {
                result = mf.GetFormListDataStyle(context, userid);
            }
            catch (Exception ex)
            {
                result.success = false;
        ...
        else if (otype == "GetFormButton")
        {
            ...
            try
            {
                ...
                DataTable table = mf.GetFormButton(iMenuID, userid);
                result.success = true;
        ...
        else if (otype == "GetFormQuery")
        {
            ...
            {
                string iFormID = context.Request.Params["iFormID"] == null ? "" : context.Request.Params["iFormID"].ToString();
                List<DataTable> tables = mf.GetFormQuery(iFormID, userid);
                result.success = true;
        ...
        else if (otype == "GetLookUpData") 
        {
            ...
            try
            {
                ...
                List<DataTable> tables = mf.GetLookUpData(lookupname, userid, rows, pageSize, filters);
                result.success = true;
                result.tables = tables;
            }
            catch (Exception ex)
        ...
        else if (otype == "getWarnRemindList")
        {
            requestResult result = new requestResult();
            remindExplain re = new remindExplain(sConnStr);
